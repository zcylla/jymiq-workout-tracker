...
COLS = [
 ...
 ('N', 'The chrome, compared', 'Glass versus two opaque plates',
  ...
  'The bar is the one place the design leaned on a platform feature that Android does not have. <b>N2 is my pick</b>: the raised plate is the same step the plates use, so the bar reads as part of the system rather than as a special case, and the lit edge is already the vocabulary. N3 is defensible but sits so close to the ground that the fade has nothing to resolve against. <b>Blur was researched and ruled out</b>, not merely skipped. On Android it is a per-frame RenderEffect re-capture rather than a live sample, and the closest published benchmark for <em>moving</em> content behind a blurred surface &mdash; which is exactly a bar over a scrolling list &mdash; is <b>+98% frame duration on a 2021 flagship</b>, already over budget at 90 or 120Hz before touching a mid-range GPU. Material&nbsp;3&rsquo;s own answer for floating chrome is a tonal opaque surface, and Google ships a switch to turn its own system blur off. N2 is not a fallback; it is what Android would have specified.',
  CHROME),
]
