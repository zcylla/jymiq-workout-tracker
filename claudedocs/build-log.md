...
`drizzle/0001_seed_exercises.sql` holds **672 exercises and 2016 muscle links**, generated by
`scripts/build-seed.mjs` from free-exercise-db (Unlicense, public domain), text only — the source's
photographs are deliberately unused. Categories kept: strength, powerlifting, olympic weightlifting,
strongman; stretching, plyometrics and cardio are dropped. `abductors` has no home in the sixteen
muscles and is dropped rather than inventing a seventeenth. `cues` and `mistakes` stay NULL: the
source has no equivalent and inventing coaching advice is not a seed script's job.

Verified on device — `/dev/db` reads EXERCISES 672.
