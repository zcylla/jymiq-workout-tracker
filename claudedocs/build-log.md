...
points into `node_modules` through the package's `./assets/*` export. `npm ci` is what fetches them.
...
npx expo run:android          # prebuild + Gradle, ~6 min
```

**The repo is on pnpm, and two of its defaults break the native build.** Both are handled by files
that are committed, so a fresh clone is fine — but if `node_modules` is ever installed by other
means, these are the failures to recognise:
...
`npx expo prebuild --platform android --clean` is the fix; `android/` is gitignored, so nothing is
lost.
